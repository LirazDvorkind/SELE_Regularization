...
base_S          = 1e4;            % middle of log-range [200, 1e7]
...
for i = 1:7
    p0 = p0_values(i);
    [SELE_p0_var(i,:), ~] = compute_sele_curve(p0, base_D, base_S, base_tau, base_alpha_scale, ...
...
for i = 1:n_vary
    D = D_values(i);
    [SELE_D_var(i,:), ~] = compute_sele_curve(base_p0, D, base_S, base_tau, base_alpha_scale, ...
        x, wavelength_PL, E_sorted, sort_idx, NC, NV, Kb, TK, C_auger, B0, ...
        n_k_wavelength, n_w_drude, k_w_drude, k_no_drude);

    S = S_values(i);
    [SELE_S_var(i,:), ~] = compute_sele_curve(base_p0, base_D, S, base_tau, base_alpha_scale, ...
        x, wavelength_PL, E_sorted, sort_idx, NC, NV, Kb, TK, C_auger, B0, ...
        n_k_wavelength, n_w_drude, k_w_drude, k_no_drude);

    tau = tau_values(i);
    [SELE_tau_var(i,:), ~] = compute_sele_curve(base_p0, base_D, base_S, tau, base_alpha_scale, ...
        x, wavelength_PL, E_sorted, sort_idx, NC, NV, Kb, TK, C_auger, B0, ...
        n_k_wavelength, n_w_drude, k_w_drude, k_no_drude);

    alpha_scale = alpha_values(i);
    [SELE_alpha_var(i,:), ~] = compute_sele_curve(base_p0, base_D, base_S, base_tau, alpha_scale, ...
        x, wavelength_PL, E_sorted, sort_idx, NC, NV, Kb, TK, C_auger, B0, ...
        n_k_wavelength, n_w_drude, k_w_drude, k_no_drude);
...
for i = 1:7
    ...
    text(x_um(peak_idx), peak_val, sprintf('  %d', i), ...
        'Color', colors_7(i,:), 'FontWeight', 'bold', 'FontSize', 10, ...
...
for i = 1:n_vary
    plot(x_um, SELE_tau_var(i,:), 'LineWidth', 2.0, 'Color', colors_4(i,:), ...
        'LineStyle', line_styles{i}, 'Marker', markers{i}, ...
        'MarkerIndices', 1:40:x_res, 'MarkerSize', 6, 'MarkerFaceColor', colors_4(i,:), ...
        'DisplayName', sprintf('$\\tau$ = %.2e s', tau_values(i)));
end
hold off;
xlabel('Depth ($\mu$m)', 'Interpreter', 'latex', 'FontSize', 11);
ylabel('SELE', 'Interpreter', 'latex', 'FontSize', 11);
title('Sensitivity to Lifetime ($\tau_{eff}$)', 'Interpreter', 'latex', 'FontSize', 12, 'FontWeight', 'bold');
grid on; grid minor;
set(gca, 'GridAlpha', 0.3, 'MinorGridAlpha', 0.15);
legend('Interpreter', 'latex', 'FontSize', 9, 'Location', 'best');
xlim([0, max(x_um)]);
