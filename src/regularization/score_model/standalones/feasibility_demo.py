...
WAVELENGTHS_NM = load_csv(str(_DATA_DIR / "wavelength_nm.csv"))
FIG_DIR = _DATA_DIR.parent / "results" / "feasibility"
...
ax_button = fig.add_axes((0.02, 0.24, 0.105, 0.05))
ax_save = fig.add_axes((0.135, 0.24, 0.105, 0.05))
...
button_resample = Button(ax_button, "↻  new curve", color="#eef1f3")
button_save = Button(ax_save, "save figure", color="#eef1f3")
...
ax_info.text(
    ...
    textwrap.fill(
        ...
        "guess you get to test. Slide it and watch every panel react. 'new curve' swaps the "
        "'true' example curve; 'save figure' exports the claim-F figure as a PNG.",
        34,
    ),
...
def ele_matches(idx: int, tol: float) -> np.ndarray:
    """Boolean mask of curves whose noiseless ELE is within relative L2 distance tol of curve idx."""
    return np.linalg.norm(E - E[idx], axis=1) / np.linalg.norm(E[idx]) < tol


def sqdist_rows(A: np.ndarray) -> np.ndarray:
    sq = (A ** 2).sum(1)
    return np.clip(sq[:, None] + sq[None, :] - 2 * A @ A.T, 0, None)


def pick_showcase_curve(tol: float, target_matches: int = 25) -> int:
    """A reference curve with enough look-alikes to show the spread without clutter."""
    sq = (E ** 2).sum(1)
    dist2 = sqdist_rows(E)
    counts = (dist2 < (tol ** 2) * sq[:, None]).sum(1)
    # Prefer curves with a surface dip and a sub-surface peak, the shape readers expect.
    peaked = (DERIVED_ALL["surf_ratio"] < 0.8) & (DERIVED_ALL["peak_pos"] > 0.5) & (DERIVED_ALL["peak_pos"] < 10)
    penalty = np.abs(counts - target_matches) + np.where(peaked, 0, N_CURVES)
...
def export_ambiguity_figure(idx: int | None = None, noise_pct: float = 1.0) -> Path | None:
    ...
    if idx is None:
        ...
        # Re-center on the ELE medoid of its look-alikes, so the reference sits inside the bundle.
        for _ in range(5):
            group = np.flatnonzero(ele_matches(idx, tol))
            medoid = group[np.sqrt(sqdist_rows(E[group])).sum(1).argmin()]
            if medoid == idx:
                break
            idx = int(medoid)
    match = ele_matches(idx, tol)
    others = match.copy()
    others[idx] = False
    if others.sum() < 1:
    ...
    ax_s.plot(Z, 100 * X[idx], color=BLACK, lw=2.0, label="reference curve")
    ...
    ax_e.plot(WAVELENGTHS_NM, 100 * E[idx], color=BLACK, lw=2.0, ls="--", label="reference curve")
    ax_e.set_ylim(0, None)
    ax_e.set_xlabel("excitation wavelength (nm)")
    ax_e.set_ylabel("ELE (%)")
    ax_e.set_title("(b) their ELEs", fontsize=10)
...
button_save.on_clicked(lambda _event: export_ambiguity_figure(state["idx"], slider_noise.val))

redraw()

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--export", action="store_true", help="save the claim-F figure and exit")
    parser.add_argument("--idx", type=int, default=None, help="reference curve (default: auto-pick)")
    parser.add_argument("--noise", type=float, default=1.0, help="ELE match tolerance in percent")
    args = parser.parse_args()
    if args.export:
        export_ambiguity_figure(args.idx, args.noise)
    else:
        plt.show()
