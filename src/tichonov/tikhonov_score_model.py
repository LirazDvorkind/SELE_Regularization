...
def solve_tikhonov(G: NDArray, J: NDArray, L: NDArray, kappa1: float, kappa2: float) -> NDArray:
    ...
    # TODO: Make sure why the size is 31 (len-1?) and not the whole thing
    K = np.vstack((G, kappa1 * L, kappa2*np.eye(S_score.size-1), L_force_last_zero))
...
def sweep_kappa(A: NDArray, B: NDArray, L: NDArray, kappa_vals1: NDArray, kappa_vals2: NDArray
                ) -> Tuple[NDArray, NDArray, List[List[NDArray]]]:
    ...
    for i, kappa1 in enumerate(kappa_vals1):
        ...
        for j, kappa2 in enumerate(kappa_vals2):
            S = solve_tikhonov(A, B, L, kappa1, kappa2)
            residuals[i, j] = np.linalg.norm(A @ S - B)
